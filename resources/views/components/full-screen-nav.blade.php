@php
    $contacts = [
        ["href" => "mailto:rammahkarpous@outlook.com", "text" => "rammahkarpous@outlook.com", "styles" => "", "medium" => "email", "target" => "_self"],
        ["href" => "tel:0683915980", "text" => "06 83 91 59 80", "styles" => "mt-2", "medium" => "phone", "target" => "_self"],
        ["href" => "https://github.com/rammahkarpous", "text" => "rammahkarpous", "styles" => "mt-6", "medium" => "github", "target" => "_blank"],
    ]
@endphp

<div class="fixed top-0 right-0 z-10 flex flex-col items-center w-20 h-screen pt-5 group">
    <a href="/{{ app()->getLocale() }}" class="fixed z-10 left-10 top-10"><x-application-logo class="w-20 h-20 text-gray-500 fill-current"/></a>
                ...
                <ul>
                    @foreach ($contacts as $contact)
                        <li>
                            <a 
                                href="{{ $contact['href'] }}"
                                class="text-xl hover:text-primary transition-all flex gap-3 items-center {{ $contact['styles'] }}" target="{{ $contact['target'] }}">
                                
                                @if ($contact['medium'] === "email") <x-icons.mail /> @endif
                                @if ($contact['medium'] === "phone") <x-icons.phone /> @endif
                                @if ($contact['medium'] === "github") <x-icons.github /> @endif
